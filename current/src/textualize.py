...
def lookup_hpo_name(hpo_id: Union[int , str]):
...
def lookup_hpo_names(hpo_ids: Union[List[Union[int, str]], int, str]):
...
def get_list(text: Union[str, List[str]]):
    if not text:
        return []
    ...
    if organization["m__I_CODE"]:
        for i_code in organization["m__I_CODE"].split("|"):
            if i_code.startswith("GARD:"):
                ...
                gard_url = f"https://rarediseases.info.nih.gov/diseases/{gard_id}"
